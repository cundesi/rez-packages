...
def ignored(abspath):
    ...
    return any(fnmatch.filter([basename], i) for i in IGNORE + _IGNORE)
...
def main(argv):
    ...
    parser.add_argument("--install_path", type=str, default=os.getenv("REZ_BUILD_INSTALL_PATH"))
    parser.add_argument(
        "--install", type=bool, default=bool(int(os.getenv("REZ_BUILD_INSTALL", "0")))
    )
    ...
    try:
        build(opts.source_path, opts.build_path, opts.install_path, targets)
    except:
        import traceback
        traceback.print_exc()
